...
public class InterpolationSearch {
    private static <T extends Number> int interpolationSearch(T[] array, T element, int first, int last) {
        // element not present
        ...
        if (array[mid].doubleValue() > element.doubleValue())
            return interpolationSearch(array, element, first, mid - 1);
        // found element
        return mid;
    }

    @SearchAlgo
    public static <T extends Number> int interpolationSearch(T[] array, T element) {
        return interpolationSearch(array, element, 0, array.length - 1);
    }
}
